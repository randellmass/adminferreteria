...
@Component({
  selector: 'app-registro-cotizacion-orden',
  templateUrl: './registro-cotizacion-orden.component.html',
  styleUrls: ['./registro-cotizacion-orden.component.css']
})
export class RegistroCotizacionOrdenComponent implements OnInit {
      ...
      formCotizacion:FormGroup = this.fb.group({
          ...
          cotproducto_id:['',Validators.required],
                  ...
                  this.equipos.forEach( item =>{
                      ...
                      if (item['id'] == equipo) {
                         ...
                         this.compresores = result.filter( compnte =>{
                            return compnte['equipo']['cottipoaccesorio_id']==2
                         });

                         this.intercambiadores = result.filter( compnte =>{
                          return compnte['equipo']['cottipoaccesorio_id']==3
                        });
                      }
      ...
      async agregarCotizacion(){
        ...
        const registro = await this.cotClientesService.store_orden(this.cotizacion_id['id'],this.formCotizacion.value);
        if(registro['res'])
        {
            //console.log(registro['data']);
            this.cotizacion_form.emit(this.cotizacion_id['id']);
            this.errors = [];
