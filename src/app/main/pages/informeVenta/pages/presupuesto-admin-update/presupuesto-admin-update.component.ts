...
@Component({
  selector: 'app-presupuesto-admin-update',
  templateUrl: './presupuesto-admin-update.component.html',
  styleUrls: ['./presupuesto-admin-update.component.css']
})
export class PresupuestoAdminUpdateComponent implements OnInit {
  ...
  campoNoValido(campo:string){
    ...
    async cargarSelectForm(presupuesto_id:any){
      ...
      this.loading=true;
        ...
        const result_informe = await this.informeVentaService.show_prespuesto_admin(presupuesto_id);
    ...
    async editar_presupuesto(){
        ...
        const editar_presupuesto = await this.informeVentaService.update_presupuesto_admin(this.presupuesto['id'],this.formPresupuesto.value);
        if (editar_presupuesto['res'])
        {
            this.presupuesto = editar_presupuesto['data'];
            this.errors =[];
            this.formPresupuesto.reset();
            this.router.navigateByUrl('main/informeventas');
        }else{
